...
scatterplot2.JGR <- function(my.data, subset1.name, subset1.val, subset2.name,
                             subset2.val, x, y, main="", xlab="", ylab="",
                             pch=1, col="black", cex.main=1, cex.lab=1, cex=1,
                             cex.axis=1, add.lm=FALSE, add.loess=FALSE,
                             add.conf=FALSE, add.pred=FALSE,
                             conf=.95)
{
        ...
        for (j in 1:n.val2)
          {
            ind <- my.data[,subset1.name]%in%subset1.val[i] &
              my.data[,subset2.name]%in%subset2.val[j]
#            png(file=file.path(resultLocation,
#                  paste(subset1.val[i]," ",subset2.val[j],".png",sep="")),
#                width=600,height=600)
            scatterplot2(my.data[ind, x], my.data[ind, y], xName=x, yName=y,
                          main=paste(subset1.val[i], subset2.val[j], main),
                          xlab=xlab, ylab=ylab, pch=pch, col=col,
                          cex.main=cex.main, cex.lab=cex.lab, cex=cex,
                          cex.axis=cex.axis, add.lm=add.lm,
                          add.loess=add.loess, add.conf=add.conf,
                          add.pred=add.pred, conf=conf)
#            dev.off()
          }
      }
  } else if (n.val1>0) {
...
#            width=600,height=600)
        ...
        scatterplot2(my.data[ind, x], my.data[ind, y], xName=x, yName=y,
                      ...
                      cex=cex, cex.axis=cex.axis, add.lm=add.lm,
                      add.loess=add.loess, add.conf=add.conf,
                      add.pred=add.pred, conf=conf)
#        dev.off()
      }
...
#        png(file=file.path(resultLocation,
#              paste(subset2.val[i],".png",sep="")),
#            width=600,height=600)
        scatterplot2(my.data[ind, x], my.data[ind, y], xName=x, yName=y,
                      main=paste(subset2.val[i], main), xlab=xlab, ylab=ylab,
                      pch=pch, col=col, cex.main=cex.main, cex.lab=cex.lab,
                      cex=cex, cex.axis=cex.axis, add.lm=add.lm,
                      add.loess=add.loess, add.conf=add.conf,
                      add.pred=add.pred, conf=conf)
...
#          "Scatterplot.png"),
#        width=600,height=600)
    scatterplot2(my.data[,x], my.data[,y], xName=x, yName=y, main=main,
                 xlab=xlab, ylab=ylab, pch=pch, col=col, cex.main=cex.main,
                 cex.lab=cex.lab, cex=cex, cex.axis=cex.axis, add.lm=add.lm,
                 add.loess=add.loess, add.conf=add.conf, add.pred=add.pred,
                 conf=conf)
#    dev.off()
  }
