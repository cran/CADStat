                      ...
                      cp=0.01, browserResults=TRUE)
{
  ...
  ## Fit
  rpart.fit = rpart(formula=as.formula(formula),data=my.data,
  ...
  if(browserResults){ 
    dev.off()
    buildresultsXML(object=list(rpart.fit$frame),location=resultLocation,
                    title="Tree Regression Summary")
  }
